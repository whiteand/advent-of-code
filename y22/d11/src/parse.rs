...
use advent_utils::nom::{
    ...
    sequence::{self, delimited, preceded, separated_pair},
    IResult, Parser,
};
...
fn parse_int(input: &str) -> IResult<&str, Expression<'_>> {
    combinator::map(character::complete::u64, Expression::Integer).parse(input)
}
fn parse_old(input: &str) -> IResult<&str, Expression<'_>> {
    combinator::map(tag("old"), |_| Expression::Var("old")).parse(input)
}
...
fn parse_expression(input: &str) -> IResult<&str, Expression<'_>> {
    alt((parse_int, parse_old, parse_sum, parse_product)).parse(input)
}
...
fn parse_monkey(input: &str) -> IResult<&str, Monkey<'_>> {
    nom::combinator::map(
        (parse_index, parse_items, parse_operation, parse_condition),
        |(_, items, operation, condition)| Monkey::new(items, operation, condition),
    )
    .parse(input)
}
pub(crate) fn parse_monkeys(input: &str) -> IResult<&str, Vec<Monkey<'_>>> {
    separated_list1(sequence::pair(newline, newline), parse_monkey).parse(input)
}
